function [Er, Gr] = RealSpace(x, S, Q, lambda_p, box, p1, p2, rc, r_vals, field_dip_1, field_dip_2, ...
...
% Dielectric contribution to the self term
Er = 3./(4*pi*(lambda_p-1)).*S;
Gr = 15./(8*pi*(lambda_p-1)).*Q;

% Self term
Er = Er + S*field_dip_1(1);
Gr = Gr + 1/2*Q*grad_quad_2(1); % quadrupoles get a factor of 1/2 from the moment expansion
