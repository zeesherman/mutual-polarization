function [p1,p2] = NeighborList_Types(cell,Ncell,type,pair_type)

% Compute a list of particle pairs in neighboring cells matching pair types.
% 
% INPUTS
% cell = (N-by-3) index of the cell to which each particle belongs
% Ncell = (1-by-3) number of cells in each dimension
% type = (N-by-1) particle types
% pair_types = (1-by-2) types to include in the neighbor list
%
% OUTPUTS
%
% p1: ID number of the first neighbor pair particle (pair_type(1))
% p2: ID number of the second neighbor pair particle (pair_type(2))
...
for i = 1:prod(Ncell)
    ...
    if counts(i) ~= 0 
        ...
        % Keep only pair_type(1) particles
        pa = pa(type(pa)==pair_type(1));
        
        % Continue only if there are pair_type(1) particles in the current cell
        if ~isempty(pa)
                ...
                for n = -1:1
                    for o = -1:1
                        ...
                        if counts(newcell) ~= 0
                            ...
                            % Keep only pair_type(1) particles
                            pb = pb(type(pb)==pair_type(2));

                            % Continue only if there are pair_type(2)
                            % particles in the neighboring cell
                            if ~isempty(pb)
            ...
            end % end of neighbor cell loops
